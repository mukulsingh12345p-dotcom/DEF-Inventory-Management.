...
import { SCHOOLS, HEAD_OFFICE_CREDENTIALS, CENTRAL_STORE_CREDENTIALS } from './constants';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Initialize credentials with defaults, will be updated from DB
      ...
      try {
        ...
        if (credData) {
            setUserCredentials(prev => prev.map(c => {
                const override = credData.find((oc: any) => oc.school_id === c.schoolId);
                if (override) {
  ...
  const login = (schoolId: string | null, role: UserRole, password: string, userId?: string) => {
    if (role === UserRole.HEAD_OFFICE) {
      if (password === HEAD_OFFICE_CREDENTIALS.password) {
      ...
      if (password === CENTRAL_STORE_CREDENTIALS.password) {
      ...
      // User Logic: Check against fetched Employees
      const employee = employees.find(e => e.id === userId && e.schoolId === schoolId);
      if (employee) {
        // Use custom password if set, otherwise default pattern
  ...
  const updatePassword = async (schoolId: string, role: UserRole, newPass: string) => {
    setUserCredentials(prev => prev.map(c => {
      if (c.schoolId === schoolId && c.role === role) {
